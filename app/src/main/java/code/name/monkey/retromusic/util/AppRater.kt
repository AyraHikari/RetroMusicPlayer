...
object AppRater {
    ...
    private const val LAUNCHES_UNTIL_PROMPT = 2 // 5 //Min number of launches
    ...
    fun appLaunched(context: Activity) {
        ...
        if (launchCount >= LAUNCHES_UNTIL_PROMPT) {
            if (true) { // (System.currentTimeMillis() >= dateFirstLaunch + DAYS_UNTIL_PROMPT * 24 * 60 * 60 * 1000) {
                //showRateDialog(context, editor)
                //showPlayStoreReviewDialog(context, editor)
                showChannelDialog(context, editor)
            }
        }
    ...
    private fun showChannelDialog(context: Context, editor: SharedPreferences.Editor) {
        MaterialAlertDialogBuilder(context)
            .setTitle("Follow Ayra's Project")
            .setMessage("Follow my another project, enjoy the interesting Apps that nobody shared before!")
            .setPositiveButton("Telegram") { _, _ ->
                context.startActivity(
                    Intent(
                        Intent.ACTION_VIEW,
                        Uri.parse("https://t.me/AyraProject")
                    )
                )
                editor.putBoolean(DO_NOT_SHOW_AGAIN, true)
                editor.commit()
